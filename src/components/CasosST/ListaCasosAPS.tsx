  ...
  useEffect(() => {
    const obtenerCasos = async () => {
      try {
        ...
        const filtrados = todos.filter((caso: any) => caso.tipo_caso === 'APS');
        ...
        <TablaGenerica<Case>
          ...
          columns={[
            { header: "Título del caso", accessor: "titulo" },
            {
              ...
              accessor: "titulo",
              render: (caso) => {
                console.log("ID del caso:", caso._id); // ← Aquí ves si necesitas usar .$oid o no
              
                return (
                  <button
                    onClick={() => navigate(`/simulacion/${caso._id}`)} // ← Esto depende del log
